...
void DirectX11RenderAPI::InitPipelines(const std::vector<RenderPipelineDesc>& pipelineDescs)
{
...
void DirectX11RenderAPI::Draw(const Drawable& drawable, const size_t nVertices)
{
	...
	dynamic_cast<DirectX11VertexBuffer*>(this->m_pVertexBuffers[drawable.vertexBufferIndex])->Bind();
	
	if (drawable.indexBufferIndex.has_value()) {
		dynamic_cast<DirectX11IndexBuffer*>(this->m_pIndexBuffers[drawable.indexBufferIndex.value()])->Bind();
...
{
	this->m_pVertexBuffers.push_back(new DirectX11VertexBuffer(this->m_pDevice, &this->m_pDeviceContext, nVertices, vertexSize));
...
{
	this->m_pIndexBuffers.push_back(new DirectX11IndexBuffer(this->m_pDevice, &this->m_pDeviceContext, nIndices));
...
{
	this->m_pConstantBuffers.push_back(new DirectX11ConstantBuffer(this->m_pDevice, &this->m_pDeviceContext, objSize, slotVS, slotPS, shaderBindingType));
